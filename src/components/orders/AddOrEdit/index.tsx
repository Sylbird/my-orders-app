...
const AddOrEditOrder = () => {
  ...
  const isValidId: boolean = id === 'new' || !Number.isNaN(Number(id));

  if (!isValidId) {
    return <Navigate to="/" replace />;
  }
  const validID = Number(id) || 'new';

  useEffect(() => {
    if (validID !== 'new') {
      fetchAndSetData(validID);
    }
  }, [validID]);
    ...
    try {
      if (orderId) {
        ...
        const onlyNewProducts = orderProducts.filter(
          (p) =>
            !serverOrderProducts.some((sp) => sp.product_id === p.product_id)
        );
  ...
  return (
    <main>
      ...
      <OrderForm
        header={validID === 'new' ? 'Add Order' : `Edit Order ${validID}`}
        orderId={validID === 'new' ? undefined : validID}
        order={order}
